...
module.exports = function(eosjs, config={keyPrefix: 'FO'})
{
	...
	function patchEcc(ecc){
		...
		if(!ecc.PubKey){
			ecc.PubKey = ecc.PublicKey;
			ecc.PublicKey = (Q, pubkey_prefix=keyPrefix) => {
				return patchPubKey(ecc.PubKey(Q, pubkey_prefix), pubkey_prefix);
			}
	
			var fromPoint = ecc.PubKey.fromPoint;
			ecc.PubKey.fromPoint = function (Q) {
				return patchPubKey(fromPoint(Q));
			};
		
			var fromBuffer = ecc.PubKey.fromBuffer;
			ecc.PubKey.fromBuffer = function (buffer, pubkey_prefix=keyPrefix) {
				return patchPubKey(fromBuffer(buffer), pubkey_prefix);
			};
		
			var fromString = ecc.PubKey.fromString;
			ecc.PubKey.fromString = function (public_key, pubkey_prefix = keyPrefix) {
				return fromString(public_key, pubkey_prefix);
			}
		
			var fromStringOrThrow = ecc.PubKey.fromStringOrThrow;
			ecc.PubKey.fromStringOrThrow = function (public_key, pubkey_prefix = keyPrefix) {
	...
	eosjs.modules.ecc = patchEcc(ecc)
