module.exports = function(eosjs)
{
	...
	function patchPubKey(pub_key, pubkey_prefix='FO') {
	...
	ecc.PublicKey = (Q, pubkey_prefix='FO') => {
	...
	PubKey.fromBuffer = function (buffer, pubkey_prefix='FO') {
	...
	// patch signature
	ecc.verify = function(signature, data, pubkey, encoding='utf8', pubkey_prefix='FO') {
		if (encoding === true) {
		...
		signature = ecc.Signature.from(signature);
		return signature.verify(data, pubkey, encoding, pubkey_prefix);
	}

	ecc.verifyHash = function(signature, dataSha256, pubkey, encoding='hex', pubkey_prefix='FO') {
		signature = ecc.Signature.from(signature);
		return signature.verifyHash(dataSha256, pubkey, encoding, pubkey_prefix);
	}

	ecc.recover = function(signature, data, encoding='utf8', pubkey_prefix='FO') {
	...
	ecc.recoverHash = function(signature, dataSha256, encoding='hex', pubkey_prefix='FO') {
	...
	function patchSignature(signature) {
		...
		signature.verifyHash = (dataSha256, pubkey, encoding='hex', pubkey_prefix='FO') => {
		...
		signature.verify = function(data, pubkey, encoding='utf8', pubkey_prefix='FO') {
