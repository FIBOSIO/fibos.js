var FIBOS = require('..');

var { TextEncoder, TextDecoder } = require('util');
var { Api } = require('eosjs/dist/eosjs-api');
var { JsonRpc } = require('eosjs/dist/eosjs-jsonrpc');
var { JsSignatureProvider } = require('eosjs/dist/eosjs-jssig');
var path = require('path');
var fs = require('fs');
var { Action } = require('eosjs/dist/eosjs-serialize');
var { base64ToBinary } = require('eosjs/dist/eosjs-numeric');

var transaction = {
    expiration: '2018-09-04T18:42:49',
    ref_block_num: 38096,
    ref_block_prefix: 505360011,
    max_net_usage_words: 0,
    max_cpu_usage_ms: 0,
    delay_sec: 0,
    context_free_actions: [],
    actions: [
        {
            account: 'testeostoken',
            name: 'transfer',
            authorization: [
                {
                    actor: 'thegazelle',
                    permission: 'active',
                },
            ],
            data: {
                from: 'thegazelle',
                to: 'remasteryoda',
                quantity: '1.0000 EOS',
                memo: 'For a secure future.',
            },
            hex_data: `00808a517dc354cb6012f557656ca4ba102700000000000004454f530000000014466f722
        06120736563757265206675747572652e`,
        },
        {
            account: 'testeostoken',
            name: 'transfer',
            authorization: [
                {
                    actor: 'thegazelle',
                    permission: 'active',
                },
            ],
            data: {
                from: 'thegazelle',
                to: 'remasteryoda',
                quantity: '2.0000 EOS',
                memo: 'For a second secure future (multiverse?)',
            },
            hex_data: `00808a517dc354cb6012f557656ca4ba204e00000000000004454f530000000028466f722061207365636f6e642073656
        37572652066757475726520286d756c746976657273653f29`,
        },
    ],
    transaction_extensions: [],
};

var serializedTx = [
    41, 210, 142, 91, 208, 148, 139, 46, 31, 30, 0, 0, 0, 0, 2, 48, 21, 164,
    25, 83, 149, 177, 202, 0, 0, 0, 87, 45, 60, 205, 205, 1, 0, 128, 138, 81,
    125, 195, 84, 203, 0, 0, 0, 0, 168, 237, 50, 50, 0, 48, 21, 164, 25, 83,
    149, 177, 202, 0, 0, 0, 87, 45, 60, 205, 205, 1, 0, 128, 138, 81, 125,
    195, 84, 203, 0, 0, 0, 0, 168, 237, 50, 50, 0, 0,
];

var deserializedTx = {
    actions: [
        {
            account: 'testeostoken',
            authorization: [{ actor: 'thegazelle', permission: 'active' }],
            data: '',
            name: 'transfer',
        },
        {
            account: 'testeostoken',
            authorization: [{ actor: 'thegazelle', permission: 'active' }],
            data: '',
            name: 'transfer',
        },
    ],
    context_free_actions: [],
    delay_sec: 0,
    expiration: '2018-09-04T18:42:49.000',
    max_cpu_usage_ms: 0,
    max_net_usage_words: 0,
    ref_block_num: 38096,
    ref_block_prefix: 505360011,
    transaction_extensions: [],
};

var serializedActions = [
    {
        account: 'testeostoken',
        authorization: [{ actor: 'thegazelle', permission: 'active' }],
        data: '00808A517DC354CB6012F557656CA4BA102700000000000004454F530000000014466F72206120736563757265206675747572652E', // eslint-disable-line
        name: 'transfer',
    },
    {
        account: 'testeostoken',
        authorization: [{ actor: 'thegazelle', permission: 'active' }],
        data: '00808A517DC354CB6012F557656CA4BA204E00000000000004454F530000000028466F722061207365636F6E64207365637572652066757475726520286D756C746976657273653F29', // eslint-disable-line
        name: 'transfer',
    },
];

var deserializedActions = [
    {
        account: 'testeostoken',
        authorization: [{ actor: 'thegazelle', permission: 'active' }],
        data: {
            from: 'thegazelle',
            memo: 'For a secure future.',
            quantity: '1.0000 EOS',
            to: 'remasteryoda',
        },
        name: 'transfer',
    },
    {
        account: 'testeostoken',
        authorization: [{ actor: 'thegazelle', permission: 'active' }],
        data: {
            from: 'thegazelle',
            memo: 'For a second secure future (multiverse?)',
            quantity: '2.0000 EOS',
            to: 'remasteryoda',
        },
        name: 'transfer',
    },
];

describe('eosjs-api', () => {
    var api;
    var rpc;
    var fetch = async (input, init) => ({
        ok: true,
        json: async () => {
            if (input === '/v1/chain/get_raw_abi') {
                return {
                    account_name: 'testeostoken',
                    abi: '', // eslint-disable-line
                };
            }

            return transaction;
        },
    });

    beforeEach(() => {
        rpc = new JsonRpc('', { fetch });
        var signatureProvider = new JsSignatureProvider(['5JtUScZK2XEp3g9gh7F8bwtPTRAkASmNrrftmx4AxDKD5K4zDnr']);
        var chainId = '038f4b0fc8ff18a4f0842a8f0564611f6e96e8535901dd45e43ac8691a1c4dca';
        api = new Api({
            rpc,
            signatureProvider,
            chainId,
            textDecoder: new TextDecoder(),
            textEncoder: new TextEncoder()
        });
    });

    it('Doesnt crash', () => {
        assert.ok(api);
    });

    it('getAbi returns an abi', async () => {
        var response = await api.getAbi('testeostoken');
        assert.ok(response);
    });

    it('getTransactionAbis returns abis by transactions', async () => {
        var response = await api.getTransactionAbis(transaction);
        assert.greaterThan(response[0].abi.length, 0);
    });

    it('getContract returns a contract', async () => {
        var response = await api.getContract('testeostoken');
        assert.ok(response.actions);
    });

    it('deserializeTransaction converts tx from binary', () => {
        var tx = api.deserializeTransaction(serializedTx);
        assert.deepEqual(tx, deserializedTx);
    });

    it('serializeActions converts actions to hex', async () => {
        var response = await api.serializeActions(transaction.actions);
        assert.deepEqual(response, serializedActions);
    });

    it('deserializeActions converts actions from hex', async () => {
        var response = await api.deserializeActions(serializedActions);

        assert.deepEqual(response, deserializedActions);
    });

    it('hasRequiredTaposFields returns true, if required fields are present', () => {
        var response = api.hasRequiredTaposFields(transaction);

        assert.isTrue(response);
    });

    it('rawAbiToJson returns correct Json from raw Abi', async () => {
        var expected = await api.getAbi('testeostoken');
        var response = await rpc.getRawAbi('testeostoken');
        var actual = api.rawAbiToJson(response.abi);

        assert.deepEqual(actual, expected);
    });

    it('jsonToRawAbi returns correct raw Abi from Json', async () => {
        var response = await rpc.getRawAbi('testeostoken');
        var expected = response.abi;
        var jsonAbi = await api.getAbi('testeostoken');
        var actual = api.jsonToRawAbi(jsonAbi);

        assert.deepEqual(actual, expected);
    });

    it('confirms jsonToRawAbi and rawAbiToJson are reciprocal', async () => {
        var expectedJsonAbi = await api.getAbi('testeostoken');
        var response = await rpc.getRawAbi('testeostoken');
        var expectedRawAbi = response.abi;
        var jsonAbi = api.rawAbiToJson(api.jsonToRawAbi(expectedJsonAbi));
        var rawAbi = api.jsonToRawAbi(api.rawAbiToJson(expectedRawAbi));

        assert.deepEqual(rawAbi, expectedRawAbi);
        assert.deepEqual(jsonAbi, expectedJsonAbi);
    });

    describe('Abi Serialization', () => {
        var serializedAbis = {
            '1.0': '',
            '1.1': '',
            '1.2': '',
        };

        it('deserializes/serializes the 1.0 abi', () => {
            var raw = base64ToBinary(serializedAbis['1.0']);
            var deserializedAbi = api.rawAbiToJson(raw);
            var serializedAbi = api.jsonToRawAbi(deserializedAbi);
            assert.deepEqual(serializedAbi, raw);
        });

        it('deserializes/serializes the 1.1 abi', () => {
            var raw = base64ToBinary(serializedAbis['1.1']);
            var deserializedAbi = api.rawAbiToJson(raw);
            var serializedAbi = api.jsonToRawAbi(deserializedAbi);
            assert.deepEqual(serializedAbi, raw);
        });

        it('deserializes/serializes the 1.2 abi', () => {
            var raw = base64ToBinary(serializedAbis['1.2']);
            var deserializedAbi = api.rawAbiToJson(raw);
            var serializedAbi = api.jsonToRawAbi(deserializedAbi);
            assert.deepEqual(serializedAbi, raw);
        });
    });

    describe('Api shorthand design (JsonAbi)', () => {
        it('errors if abi is not cached', () => {
            var abiCheck = () => {
                api.with('testeostoken').as('bob')
                    .transfer('thegazelle', 'remasteryoda', '1.0000 EOS', 'For a secure future.');
            };
            assert.throws(abiCheck);
        });

        it('generates a valid serialized action using api.with()', async () => {
            await api.getAbi('testeostoken');

            var serializedAction = api.with('testeostoken').as('thegazelle')
                .transfer('thegazelle', 'remasteryoda', '1.0000 EOS', 'For a secure future.');
            assert.deepEqual(serializedAction, serializedActions[0]);
        });

        it('generates a valid serialized action using tx.with()', async () => {
            await api.getAbi('testeostoken');

            var tx = api.buildTransaction();
            var serializedAction = tx.with('testeostoken').as('thegazelle')
                .transfer('thegazelle', 'remasteryoda', '2.0000 EOS', 'For a second secure future (multiverse?)');
            assert.deepEqual(serializedAction, serializedActions[1]);
        });

        it('confirms serializeActions and ActionBuilder return same serialized data', async () => {
            var response = await api.serializeActions(transaction.actions);

            var firstAction = api.with('testeostoken').as('thegazelle')
                .transfer('thegazelle', 'remasteryoda', '1.0000 EOS', 'For a secure future.');

            var secondAction = api.with('testeostoken').as('thegazelle')
                .transfer('thegazelle', 'remasteryoda', '2.0000 EOS', 'For a second secure future (multiverse?)');

            assert.deepEqual([firstAction, secondAction], response);
        });

        it('generates the same serialized data using the longer authorization', async () => {
            await api.getAbi('testeostoken');

            var firstSerializedAction =
                api.with('testeostoken').as('thegazelle')
                    .transfer('thegazelle', 'remasteryoda', '1.0000 EOS', 'For a secure future.');
            var secondSerializedAction =
                api.with('testeostoken').as([{ actor: 'thegazelle', permission: 'active' }])
                    .transfer('thegazelle', 'remasteryoda', '1.0000 EOS', 'For a secure future.');

            assert.deepEqual(firstSerializedAction, secondSerializedAction);
        });

        it('confirms the transaction extension serialization is reciprocal', async () => {
            var resourcePayerTrx = {
                expiration: '2021-06-28T15:55:37.000',
                ref_block_num: 2097,
                ref_block_prefix: 1309445478,
                actions: [{
                    account: 'eosio.token',
                    name: 'transfer',
                    authorization: [{
                        actor: 'bob',
                        permission: 'active',
                    }, {
                        actor: 'alice',
                        permission: 'active',
                    }],
                    data: '0000000000000E3D0000000000855C34010000000000000004535953000000000E7265736F75726365207061796572'
                }],
                context_free_actions: [],
                resource_payer: {
                    payer: 'payer',
                    max_net_bytes: 4096,
                    max_cpu_us: 250,
                    max_memory_bytes: 0
                }
            };
            var serialized = [[1, '0000000080ABBCA90010000000000000FA000000000000000000000000000000']];
            var deserialized = {
                resource_payer: {
                    payer: 'payer',
                    max_net_bytes: 4096,
                    max_cpu_us: 250,
                    max_memory_bytes: 0
                }
            };

            var serializedTransactionExtensions = api.serializeTransactionExtensions(resourcePayerTrx);
            assert.deepEqual(serializedTransactionExtensions, serialized);
            var deserializedTransactionExtensions = api.deserializeTransactionExtensions(serialized);
            assert.deepEqual(deserializedTransactionExtensions, deserialized);
        });
    });
});

require.main === module && test.run(console.DEBUG);
